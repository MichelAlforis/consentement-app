...
## 8. Composant `AppLogo` — référence technique

> Fichier : `app/components/ui/AppLogo.tsx`
...
| `className` | `string` | — | Classes CSS/Tailwind pour contrôler la taille |
...
- **`dark`** — dégradé violet clair → violet moyen. Fond rect semi-transparent blanc 8 %. Utilisé sur SplashScreen, PremiumScreen.
- **`light`** — dégradé rose → violet → teal. Fond rect blanc 78 %. Utilisé sur WelcomeScreen, LanguageScreen, OnboardingWizard.
- **`theme`** — lit `colors.accent`, `colors.premium`, `colors.success` depuis `ThemeContext`. Partout dans l'app une fois le thème sélectionné.

### Animation `animated={true}`

- `scale: [1, 1.04, 1]` + `rotate: [0, -1.5°, 0, 1.5°, 0]`
- Durée 5,5 s, loop infini, `easeInOut`
- Ne pas doubler : si un `motion.div` parent anime déjà `scale`, passer `animated={false}` sur le logo.
...
| DuoWaitingStep | `w-20 h-20` | `theme` | ✗ (conteneur pulse) |
| DuoConnectedStep | `w-16 h-16` | `theme` | ✅ |
| DuoReadyStep | `w-9 h-9` | `theme` | conditionnel (`bothReady`) |
| DuoSummaryStep | `w-16 h-16` | `theme` | ✅ |
| DuoRevealStep | `w-12 h-12` | `theme` | ✅ |

### Ce qui n'utilise pas `AppLogo` — intentionnel

Les `Heart` de lucide-react restent dans : CardGame (favoris), GooseGame (case Complicité), HelpScreen (bienveillance), DuoRevealStep (compteur zones).
