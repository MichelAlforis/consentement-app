export const pratiquesBase = {
  pratiquesBase: {
    fiches: {
      title: 'Pratiques de base',
      subtitle: 'Définition, consentement et loi — pour nommer les choses par leur nom.',
      sectionDef: 'Définition',
      sectionConsent: 'Consentement',
      sectionLoi: 'Ce que dit la loi',
      sectionQuestion: 'Question à se poser',
      markRead: "J'ai lu — Voir ma carte",
      0: {
        titre: 'Fellation',
        definition: 'Stimulation orale du pénis. Pratique courante dans les relations hétérosexuelles et homosexuelles masculines.',
        consentement: 'Le consentement doit être explicite et réitéré — notamment pour l\'éjaculation (lieu, moment). La personne qui donne peut s\'arrêter à tout moment sans se justifier. La pression physique (maintien de la tête) sans accord préalable est une agression.',
        loi: 'Imposer une fellation sans consentement constitue un viol (art. 222-23 CP), puni de 15 ans de réclusion criminelle. La contrainte peut être physique ou morale.',
        question: 'Avons-nous parlé de ce qui est ok et de ce qui ne l\'est pas pour cette pratique ?',
      },
      1: {
        titre: 'Cunnilingus',
        definition: 'Stimulation orale de la vulve et du clitoris. Pratique courante dans les relations hétérosexuelles et homosexuelles féminines.',
        consentement: 'Le consentement doit être explicite — notamment sur le rythme, la pression et les zones concernées. La personne qui reçoit garde le contrôle à tout moment et peut demander l\'arrêt sans se justifier.',
        loi: 'Imposer un cunnilingus sans consentement constitue un viol (art. 222-23 CP), puni de 15 ans de réclusion criminelle.',
        question: 'As-tu demandé ce que l\'autre apprécie et ce qu\'il ou elle n\'aime pas ?',
      },
      2: {
        titre: 'Masturbation mutuelle',
        definition: 'Stimulation manuelle des organes génitaux de l\'autre, simultanément ou alternativement. Pratique présente dans toutes les orientations sexuelles.',
        consentement: 'Chaque personne guide l\'autre sur les zones, le rythme et la pression qu\'elle apprécie. La communication pendant — verbale ou non-verbale — est essentielle pour que les deux personnes soient à l\'aise.',
        loi: 'Tout acte sexuel sans consentement, y compris la masturbation forcée, est une agression sexuelle (art. 222-22 CP). La pénétration digitale imposée constitue un viol (art. 222-23 CP).',
        question: 'L\'autre m\'a-t-il ou elle guidé·e sur ce qu\'il ou elle aime, ou ai-je supposé ?',
      },
      3: {
        titre: 'Pénétration vaginale',
        definition: "Introduction du pénis ou d'un accessoire dans le vagin. Pratique centrale dans beaucoup de relations hétérosexuelles et certaines relations lesbiennes.",
        consentement: 'Le consentement doit être clairement donné avant et maintenu pendant. Vérifier régulièrement que l\'autre est toujours à l\'aise est normal et sain. La douleur doit être signalée et entendue immédiatement.',
        loi: 'La pénétration sans consentement est un viol (art. 222-23 CP), puni de 15 ans de réclusion. Si la victime a moins de 15 ans, la peine monte à 20 ans de réclusion.',
        question: 'Avons-nous parlé de contraception et de protection avant de commencer ?',
      },
      4: {
        titre: 'Sodomie',
        definition: "Pénétration anale par le pénis ou un accessoire. Pratique présente dans toutes les orientations sexuelles. La zone anale ne produit pas de lubrification naturelle.",
        consentement: "Le consentement doit être explicite, préparé et maintenu à chaque étape. Un arrêt demandé doit être respecté immédiatement. La douleur n'est pas normale — elle signale que quelque chose ne va pas et nécessite une pause ou un arrêt.",
        loi: "La pénétration anale sans consentement est un viol (art. 222-23 CP), puni de 15 ans de réclusion criminelle. Aucune pratique sexuelle n'est 'moins grave' parce qu'elle est non-vaginale.",
        question: 'Avons-nous discuté de la préparation, du rythme et des mots pour dire stop avant de commencer ?',
      },
    },
  },
};
