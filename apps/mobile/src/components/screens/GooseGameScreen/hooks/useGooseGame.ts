...
import { usePersist } from '../../../../game-engine/shared/usePersist';
import { useHaptics } from '../../../../game-engine/shared/useHaptics';
import { useDice } from './useDice';
import { usePawnAnimation } from './usePawnAnimation';
import { useConfetti } from './useConfetti';
import type { Player, Phase, TurnStep, SavedGooseGame } from '../types';
...
export function useGooseGame({ isAdult }: { isAdult: boolean }) {
  ...
  const persist = usePersist<SavedGooseGame>(SAVE_KEY);
  ...
  const persistGame = useCallback((_p0: number, _p1c: number, _cp: 0 | 1, _ac: number) => {
    ...
    if (!pl1 || !pl2) return;
    persist.save({ players: [pl1, pl2], positions: [_p0, _p1c], currentPlayer: _cp, accordsCount: _ac });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // ── Setup ──────────────────────────────────────────────────────────────────
  const handleP1Confirm = useCallback((name: string, pawn: Player['pawn']) => {
    setP1({ name, pawn });
    setPhase('setup-p2');
  }, []);

  const handleP2Confirm = useCallback((name: string, pawn: Player['pawn']) => {
    setP2({ name, pawn });
    setPhase('pacte');
  }, []);
  ...
  const startNewGame = useCallback(() => {
    ...
    setPhase('playing');
    persist.save({ players: [pl1, pl2], positions: [0, 0], currentPlayer: 0, accordsCount: 0 });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // ── Valeurs dérivées ───────────────────────────────────────────────────────
  const player1 = p1 ?? { name: 'Joueur 1', pawn: 'Zap' as Player['pawn'] };
  const player2 = p2 ?? { name: 'Joueur 2', pawn: 'Leaf' as Player['pawn'] };
